...
public class SimplexSolver {
    ...
    public Resultado resolverSimplex(FuncionObjetivo fo, Restriccion[] restricciones) {
        try {
            ...
            for (int j = 0; j < n; j++) {
                if (fo.isMaximizar()) {
                    tabla[m][j] = -fo.getCoeficientes()[j];
                } else {
                    tabla[m][j] = fo.getCoeficientes()[j];
                }
            }
            ...
            System.err.println("Error en SimplexSolver: " + e.getMessage());
            e.printStackTrace();
    ...
    private boolean simplex(double[][] tabla, int m, int n) {
        ...
        for (int iter = 0; iter < maxIteraciones; iter++) {
            ...
            // Si no hay valores negativos, óptimo encontrado
            if (colPivote == -1) {
                return true;
            }
            ...
            // Hacer ceros en columna pivote excepto fila pivote
            for (int i = 0; i <= m; i++) {
                if (i != filaPivote) {
